$(document).ready(function() {
    ...
    $("#Validator").on("hidden.bs.modal", function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (response["status"] == "fail") {
                    ...
                    $.ajax({
                        type: 'get',
                        url: baseUrl+"Transaction/GetAccountBalance",
                        data: {Id : token.val()},
                        dataType: "json",
                        success: function(data) {
                            var response = jQuery.parseJSON(data);
                            if (response["status"] == "pass") {
                                responseMessage("The balance in your account is: <span>&#163</span>" + response["token"], "alert alert-success")
                                $("#task").hide();
                                $("#afterTask").show();
                            } else {
